...
def parse_args():
    parser = argparse.ArgumentParser(
        description='MapDiffusion DDIM test (and eval) a model')
    parser.add_argument('config', help='test config file path')
    parser.add_argument('checkpoint', type=str, help='checkpoint file')
    parser.add_argument('--work-dir', help='the dir to save logs and models')
    parser.add_argument('--sampling_timesteps', help='Number of DDIM sampling steps')
    parser.add_argument('--eta', help='Eta parameter in DDIM diffusion sampling')
    parser.add_argument('--query_threshold', help='Threshold for which to drop queries in sampling loop')
...
def main():
    ...
    # embed()
    # compute noise coef
    total_steps = cfg.get("total_steps", None)
    print(f"Diffusion Total Steps: {total_steps}")
    scheduler = cfg.get("scheduler", None)
    print(f"Diffusion Noise Scheduler: {scheduler}")
    if total_steps is not None and scheduler is not None:
        from plugin.models.utils.coef import compute_ddpm_coef
        coef = compute_ddpm_coef(total_steps, scheduler)
    else:
        coef = None
    eta = float(args.eta) if args.eta is not None else None
    sampling_timesteps= int(args.sampling_timesteps) if args.sampling_timesteps is not None else None
    query_threshold = float(args.query_threshold) if args.query_threshold is not None else None
    ...
    if not distributed:
        ...
        outputs = multi_gpu_test(model, data_loader, total_steps, coef, eta, sampling_timesteps, query_threshold, args.tmpdir,
                                args.gpu_collect)
