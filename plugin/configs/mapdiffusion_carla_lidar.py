...
# bev configs
# Square, following the export's own tile size. CarlaDataset asserts this
# against the `tile_radius` recorded in the annotation pkl, so pointing this
# config at the 60m export fails loudly rather than silently rescaling every
# map element. For that export: tile_radius = 30.0.
tile_radius = 12.5
roi_size = (2 * tile_radius, 2 * tile_radius) # bev range, 25m x 25m tile
bev_h = 100
bev_w = 100
    ...
    backbone_cfg=dict(
        ...
        lidar_encoder=dict(
            ...
            backbone=dict(
                ...
                in_channels=4,
                # (nz, ny, nx). This order, and the encoder_paddings below
                # putting the odd padding on the z axis, follow from stock
                # mmdet3d/mmcv emitting voxel coords as (batch, z, y, x) --
                # the vendored fork in the sibling GeMap/MapTRv2 trees is
                # transposed and their configs read [nx, ny, nz]. Verified
                # with tools/misc/probe_lidar_encoder.py, along with the
                # 3200 below; do not hand-derive either after changing the
                # range or voxel size.
                sparse_shape=[420, 250, 250],
                output_channels=128,
                order=('conv', 'norm', 'act'),
        ...
        # the encoder's downsampling, measured at (B, 3200, 32, 32).
        lidar_bev_proj=dict(
            type='ConvFuser',
            in_channels=[3200],
...
train_pipeline = [
    ...
    *lidar_pipeline,
    ...
    dict(type='Collect3D', keys=['points', 'vectors', 'gts'], meta_keys=(
