...
def custom_multi_gpu_test_diffuse(model, data_loader, t, coef, eta, sampling_timesteps, query_threshold, tmpdir=None, gpu_collect=False):
    ...
    """
    print(f"Performing evaluation with diffusion parameters eta {eta}, sampling_timesteps {sampling_timesteps}, query_threshold {query_threshold}")
    model.eval()
    bbox_results = []
    mask_results = []
    ...
    for i, data in enumerate(data_loader):
        with torch.no_grad():
            result = model(timestep = t, coef=coef, eta=eta, sampling_timesteps= sampling_timesteps, query_threshold=query_threshold, return_loss=False, rescale=True, **data)
            # encode mask results
